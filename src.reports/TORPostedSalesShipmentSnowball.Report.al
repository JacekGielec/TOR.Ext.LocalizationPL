report 50463 "TOR Sales - Shipment Snowball"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src.reports/TORPostedSalesShipmentSnowball.rdlc';
    Caption = 'TOR Sales - Shipment Snowball';
        ...
        dataitem("Sales Shipment Header"; "Sales Shipment Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(ShowImage1; GetShowImage1(OutputNo, Customer."Cust. Delivery Document"))
                    {
                    }
                    column(CompanyInfo4Picture; Customer.Image)
                    {
                    }
                    column(CustDeliveryDocument; Customer."Cust. Delivery Document")
                    {
                    }
                    column(SalesShptCopyText; StrSubstNo(Text002, "Sales Shipment Header"."No." + CopyText))
                    {
                    }
                    ...
                    column(CompanyKRSCaption; CompanyKRSLbl)
                    {

                    }
                    column(CompanyKRS; companyinfo."TOR KRS")
                    {
                    }
                    column(CompanyREGONCaption; CompanyRegonLbl)
                    {

                    }
                    column(CompanyRegon; companyinfo."Registration No.")
                    {

                    }
                    column(CompanyNIPCaption; CompanyNIPLbl)
                    {

                    }
                    column(CompanyNIP; companyinfo."VAT Registration No.")
                    {
                    ...
                    dataitem(ItemTrackingLine; "Integer")
                    {
                        ...
                        column(ExpDateCaption; ExpDateCaptionLbl)
                        {
                ...
                begin
                    if Number > 1 then begin
                        ...
                        OutputNo += 1;
                        /*
                            if customer."Cust. Delivery Document" then begin
                                FormatAddressFields2("Sales Shipment Header");
                                CopyText := '';
                                //Evaluate(CompanyInfo.Picture, format(Customer.Image));
                                //CompanyInfo.Picture.CreateInStream(MyStr, TextEncoding::Windows);
                            end;
*/
                    end;
    ...
    trigger OnPreReport()
    var
        s: Record "Sales Shipment Header";
    begin
        if not CurrReport.UseRequestPage then
            InitLogInteraction;
        AsmHeaderExists := false;
        s.Get("Sales Shipment Header".getfilter("No."));
        Customer.get(s."Sell-to Customer No.");
        if not customer."Cust. Delivery Document" then
            CurrReport.Quit();

        CompanyInfo.Name := 'Zakład Produkcyjny';
        CompanyInfo."TOR KRS" := '0000007819';
        CompanyInfo."VAT Registration No." := '732-17-34-933';
        CompanyInfo."Registration No." := '471647640';
    ...
    var
        ...
        ExpDateCaptionLbl: Label 'Expiration Date';
        ...
        CompanyKRSLbl: Label 'KRS: ';
        CompanyNIPLbl: Label 'NIP: ';
        CompanyRegonLbl: Label 'REGON: ';
        Reciver_CaptionLbl: Label 'Podpis osoby uprawnionej do otrzymania towaru';
        Drawer_CaptionLbl: Label 'Podpis osoby wystawiającej';
        Warehouseman_CaptionLbl: Label 'Podpis osoby uprawnionej do wydania towaru';
        UserName: Text;
        User: Record User;
    ...
    local procedure FormatAddressFields2(SalesShipmentHeader: Record "Sales Shipment Header")
    var
    ...
    begin
        ...
        i := 1;
        CompanyAddr[i] := SalesShipmentHeader."Sell-to Customer Name";

        if SalesShipmentHeader."Sell-to Customer Name 2" <> '' then begin
            i := i + 1;
            CompanyAddr[i] := SalesShipmentHeader."Sell-to Customer Name 2";
        end;

        if SalesShipmentHeader."Sell-to Address" <> '' then begin
            i := i + 1;
            CompanyAddr[i] := SalesShipmentHeader."Sell-to Address";
        end;
        if SalesShipmentHeader."Sell-to Address 2" <> '' then begin
            i := i + 1;
            CompanyAddr[i] := SalesShipmentHeader."Sell-to Address 2";
        end;
        if SalesShipmentHeader."Sell-to post code" <> '' then begin
            i := i + 1;
            CompanyAddr[i] := SalesShipmentHeader."Sell-to post code";

        end;
        if SalesShipmentHeader."Sell-to City" <> '' then begin
            if SalesShipmentHeader."Sell-to post code" <> '' then
                CompanyAddr[i] := CompanyAddr[i] + ' ';
            CompanyAddr[i] := CompanyAddr[i] + SalesShipmentHeader."Sell-to City";
        end;
        if SalesShipmentHeader."Sell-to county" <> '' then begin
            i := i + 1;
            CompanyAddr[i] := SalesShipmentHeader."Sell-to county";
        end;
    ...
    procedure GetShowImage1(outputno: Integer; custdeliverydocument: Boolean): Boolean
    begin
        if not CompanyInfo3.Picture.HasValue then
            exit(false);
        if custdeliverydocument = true then
            exit(false);
        exit(true);
    end;

    procedure ShowImage2(): Boolean
    begin
        EXIT(true);
    end;
