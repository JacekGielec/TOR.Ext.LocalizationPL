report 50468 "TOR Vendor Reconciliation"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src.reports/VendorReconciliation.rdl';
        ...
        dataitem(Vendor; Vendor)
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(ContractorNo; Vendor."No.")
                    {
                    }
                    column(ContractingAddr1; VendAddr[1])
                    {
                    }
                    column(ContractingAddr2; VendAddr[2])
                    {
                    }
                    column(ContractingAddr3; VendAddr[3])
                    {
                    }
                    column(ContractingAddr4; VendAddr[4])
                    {
                    }
                    column(ContractingAddr5; VendAddr[5])
                    {
                    }
                    column(ContractingAddr6; VendAddr[6])
                    {
                    }
                    column(ContractingAddr7; VendAddr[7])
                    {
                    }
                    column(ContractingAddr8; VendAddr[8])
                    {
                    }
                    ...
                    column(VendAddr10; VendAddr[10])
                    {
                    }
                    column(VendAddr11; VendAddr[11])
                    {
                    }
                    column(VendAddr12; VendAddr[12])
                    {
                    }
                    ...
                    dataitem(TotalInCurrency; "Integer")
                    {
                        ...
                        begin
                            ...
                            if PrintAmountsInCurrency and (CurrencyBuf.Code <> '') then begin
                                ...
                                DebitCreditCalc(CustomerNo, Vendor."No.", CurrencyBuf.Code, ReconcileDate, false);
                            end else
                                DebitCreditCalc(CustomerNo, Vendor."No.", '', ReconcileDate, true);
                        end;
                    }
                    dataitem(Footer; "Integer")
                    {
                        DataItemTableView = SORTING(Number) WHERE(Number = CONST(1));
                        column(BalanceAcceptance_Caption; STRSUBSTNO(Text007, ReturnDate) + Text008 + STRSUBSTNO(Text009, CompanyInfo."Phone No."))
                        {
                        }
                        column(OurFax_Caption; STRSUBSTNO(Text006, CompanyInfo."Fax No."))
                        {
                        }
                ...
                begin
                    if Number = 1 then begin
                        CopyText := Text016;
                        ChangeCustCompanyAddr(CompanyAddr, VendAddr)
                    end else begin
                        CopyText := Text017;
                        ChangeCustCompanyAddr(VendAddr, CompanyAddr);
                        OutputNo += 1;
                        CompAddr1[1] := VendAddr1[1];
                    end;
                end;
            ...
            begin
                ...
                VendAddr[9] := Vendor."Phone No.";
                VendAddr[10] := Vendor."Fax No.";
                VendAddr[11] := Vendor."E-Mail";
                VendAddr[12] := Vendor."VAT Registration No.";

                FormatAddr.Vendor(VendAddr1, Vendor);
...
*/
                ...
                TotalAmountLCY := CVMgt.CalcCVDebt(CustomerNo, Vendor."No.", '', ReconcileDate, true);
                ...
                CVMgt.FillCVBuffer(CurrencyBuf, CVLedgEntry, CustomerNo, Vendor."No.", ReconcileDate, PrintAmountsInCurrency);
            end;
        }
    }
    ...
    var
        ...
        AddrInfo: Text[250];
        CourtInfo: Text[250];
        VendAddr: array[12] of Text[100];
        VendAddr1: array[12] of Text[100];
        ...
        ContractorNo_CaptionLbl: Label 'Vendor No.';
    ...
    begin
        ...
        VendAddr[1] := CustomerData[1];
        VendAddr[2] := CustomerData[2];
        VendAddr[3] := CustomerData[3];
        VendAddr[4] := CustomerData[4];
        VendAddr[5] := CustomerData[5];
        VendAddr[6] := CustomerData[6];
        VendAddr[7] := CustomerData[7];
        VendAddr[8] := CustomerData[8];
    end;
