report 50462 "TORReBookingOfManuf. Costs"
{
    //AdditionalSearchTerms = 'cost forwarding';
    ApplicationArea = Basic, Suite;
    Caption = 'TOR Re-booking Of Manufacturing Costs';
    ...
    begin
        if ReBooking.FindFirst() then
            repeat
                ReBooking.TestField("Prod. Cost G/L Account 501");
                ReBooking.TestField("Prod. Cost G/L Account No.");
                GLAccount.SetFilter("No.", '%1', ReBooking."Prod. Cost G/L Account 501");
                GLAccount.SetRange("Account Type", GLAccount."Account Type"::Posting);
                ...
                if GLAccount.FindFirst() then
                    repeat
                        ...
                        if CostAmount <> 0 then begin
                            CreateJnlLine(GLAccount."No.", ReBooking."Prod. Cost G/L Account No.", -costamount);
                        end;
    ...
    begin
        ...
        GenJnlLine.Description := StrSubstNo('Przeks. kosztów wytworzenia m-c %1', Format(DateTo, 0, '<Month,2>-<Year4>'));
